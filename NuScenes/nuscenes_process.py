...
def process_scene(scene):
    ...
    while sample['next']:
        ...
        for i,annotation in enumerate(annotations):
            ...
            ### Save HDMap RGB Img per agent and sample (N_samples, N_agents, 3,W,H) and then save only HDmap[current_frame] for each seq  ->  [N_seq,max_num_agents,3,W,H]
            #img = input_representation.make_input_representation(instance_token, sample_token) #plt.imsave()  #[500,500,3]
            #plt.imshow(img)
            #file_name = os.path.join(base_path, 'samples', sample_token+'_'+instance_token+'.png')
...
ns_scene_names = dict()
ns_scene_names['train'] = get_prediction_challenge_split("mini_train", dataroot=DATAROOT) 
ns_scene_names['val'] =  get_prediction_challenge_split("mini_val", dataroot=DATAROOT)
...
for data_class in ['train', 'val']:
    ...
    save_path = '/home/sandra/PROGRAMAS/DBU_Graph/NuScenes/nuscenes_mini_' + data_class + '.pkl'
