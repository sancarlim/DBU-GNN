...
class LitGNN(pl.LightningModule):
    ...
    def frange_cycle_linear(self, start=0.0, stop=1.0, ratio=0.5, period=500):
        #period = n_iter/n_cycle
        step = (stop-start)/(period*ratio) # linear schedule
        
        if self.beta_constant == False and self.global_step % 250 == 0:
            self.beta = stop 
            self.beta_constant = True
        elif self.beta_constant == True and self.global_step % 500 == 0:
            self.beta = start
            self.beta_constant = False
        elif self.beta_constant == False:
            self.beta += step
        else:
            self.beta == stop
    ...
    def vae_loss_prior(self, pred, gt, mask, mu, log_var, mu_prior, log_var_prior, z0):
        ...
        return loss, {'loss': loss, 'Reconstruction_Loss':recons_loss, 'KL':self.beta *kld_loss.mean(), 'KL_prior': self.beta *kld_prior.mean(), 'z0': self.gamma *z0_loss, 'beta': self.beta}
    ...
    def test_step(self, test_batch, batch_idx):
        ...
        for i in range(5): # @top10 Saco el min ADE/FDE por escenario tomando 15 muestras (15 escenarios)
            ...
            preds = self.model.inference(batched_graph, feats,e_w,snorm_n,snorm_e, maps)   #,mu,logvar
...
def main(args: Namespace):
    ...
    if not args.nowandb:
        ...
        trainer = pl.Trainer( weights_summary='full', gpus=args.gpus, deterministic=True, precision=16, log_every_n_steps=5, logger=wandb_logger, callbacks=[early_stop_callback,checkpoint_callback], profiler=True)  # resume_from_checkpoint=config.path, precision=16, limit_train_batches=0.5, progress_bar_refresh_rate=20,
    else:
